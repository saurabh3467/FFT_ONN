...
# Load the data from a text file with a file name that has spaces
filename = r"C:\Users\SoniS\Dropbox\Academia\RuG_PhD\GitHub\FFT_EGaIn_ONN\inputfile\230202_1e_4cm_100uL_1_10V.txt"
...
data = np.loadtxt('modified_data.txt', dtype=str, delimiter=';')
...
# Save the FFT data to a text file
with open('3mL_2M_NaOH_backgroud_line_2cm_Graphite_electrode_1V_fft_data.txt', 'w') as f:
    for i in range(len(freq)):
        f.write('{} {}\n'.format(freq[i], np.abs(fft_current[i])))

plt.show()  # Use plt.show() to display the plot

volume = input("Do you want to close the plot now? (y/n): ")
